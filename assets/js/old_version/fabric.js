...
let rootCanvas = new fabric.Canvas('canvas', {selection: false});
...
function draw_grid(grid) {
  ...
  for (let i = 0; i < rowCount; i++) {
    rootCanvas.add(new fabric.Line([ i * grid, 0, i * grid, H], { stroke: '#ccc', selectable: false }));
  }
  for (let i = 0; i < colCount; i++) {
    rootCanvas.add(new fabric.Line([ 0, i * grid, W, i * grid], { stroke: '#ccc', selectable: false }));
  }
}
